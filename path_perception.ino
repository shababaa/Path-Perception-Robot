/* EDITED
 * Copyright (C) 2023 dBm Signal Dynamics Inc.
 *
 * File:            running-the-fairway-complete-collision-detect.ino
 * Project:         Tech Briefs #3 - Add Collision Detection to the RPi Pico W Robot (19-00030A)         
 * Date:            April 28, 2023
 * 
 * "Running the Fairway" Demo with Collision Detection (IoT enabled challenge)
 * 
 * Robot implements the "Running the Fairway - Collision Detect" challenge, and connects with an Adafruit IO Dashboard
 *
 * Hardware Configuration: CETA IoT Robot (Schematic #14-00069A, based on RPi Pico W) with mounted HCSR04 Ultrasonic Sensor
 * 
 * HCSR04 <--> CETA IoT Robot Hardware Connections:
 * 
 * VCC  -->   3.3V Bus
 * GND  -->   GND
 * ECHO -->   Pico W Pin GP16
 * TRIG -->   Pico W Pin GP17
 *
*/

/*** Include Files ************************************************************/
#include <ArduinoJson.h>        // Required for JSON deserialization
#include <Servo.h>              // Required for the Servo library APIs
#include <EEPROM.h>             // Required for flash EEPROM emulation to store OPTO calibration values
#include <HCSR04.h>             // Required for HC-SR04 library
#include "mqttc.h"              // MQTT Client functions

/*** Symbolic Constants used in this module ***********************************/

// Digital I/O Pin Function Assignments
#define USER_LED_PIN 14         // OUTPUT - connected to USER LED (D1) 
#define USER_SWITCH_PIN 15      // INPUT - connected to USER SWITCH (S2)
...
#define LEFT_OPTO_PIN A2        // Left opto connected to AIN2
#define MIDDLE_OPTO_PIN A1      // Middle opto connected to AIN1
#define RIGHT_OPTO_PIN A0       // Right opto connected to AIN0

// Optosensor sampling intervals
#define OPTO_SAMPLE_INTERVAL 10	        // sample interval (in mS) for sampling the sensors
#define OPTO_CALIBRATE_INTERVAL 10000   // averaging time for opto calibration (in mS)

// Ultrasonic collision sensor definitions
#define COLLISION_SAMPLE_INTERVAL 100   // sample interval for collision sensor
...
#define RIGHT_SERVO_DEFAULT_STRAIGHT 18    
#define LSERVO_UPPER_SPEED_LIMIT (LEFT_SERVO_DEFAULT_STRAIGHT + 6)
#define LSERVO_LOWER_SPEED_LIMIT (LEFT_SERVO_DEFAULT_STRAIGHT - 6)
#define RSERVO_UPPER_SPEED_LIMIT (RIGHT_SERVO_DEFAULT_STRAIGHT + 6)
#define RSERVO_LOWER_SPEED_LIMIT (RIGHT_SERVO_DEFAULT_STRAIGHT - 6)
...
void Initialize(void){
  ...
  Serial.begin(115200);
  Serial.println("Tech Briefs #3: \"Running the Fairway\" Demo (IoT) with Collision Detect)\r\n");
  ...
  // USER SWITCH (S2)
  pinMode(USER_SWITCH_PIN, INPUT);                    // set digital pin as input
...
void RobotWait(void){
  ...
  else if(mqttcRxIsAvailable((String)SUB_TOPIC_REMOTE_PUSHBUTTON)){
...
void RobotTurnAround(void){
  ...
  delay(1000);                                    // ensure optos are not positioned over line                                      
...
// {"feeds": {"left_opto": 656, "middle_opto": 457, "right_opto": 345, "left_opto_trip": 750, "middle_opto_trip": 755, "right_opto_trip": 765, "collision_distance": 9.62, "robot_state": "IDLE"}}
...
void eventDeserialize(void){
  ...
  // Use the "ArduinoJSON Assistant" to calculate the size of the JSON document using the Payload Prototype
  // Go to: https://arduinojson.org/v6/assistant/
